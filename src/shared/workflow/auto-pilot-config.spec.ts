...
describe("createDefaultAutoPilotConfig", () => {
  it("defaults to 10 re-loops and a 4s settle delay", () => {
    expect(createDefaultAutoPilotConfig()).toEqual({ reloopLimit: 10, settleDelayMs: 4000 });
  });

  it("returns a fresh object each call (no shared reference)", () => {
    const a = createDefaultAutoPilotConfig();
    a.reloopLimit = 9;
    expect(createDefaultAutoPilotConfig().reloopLimit).toBe(10);
  });
});

describe("clampAutoPilotConfig", () => {
  it("fills missing fields from the default", () => {
    expect(clampAutoPilotConfig({})).toEqual({ reloopLimit: 10, settleDelayMs: 4000 });
  });

  it("clamps reloopLimit into 1..50 and floors it to an integer", () => {
    expect(clampAutoPilotConfig({ reloopLimit: 0 }).reloopLimit).toBe(1);
    expect(clampAutoPilotConfig({ reloopLimit: 99 }).reloopLimit).toBe(50);
