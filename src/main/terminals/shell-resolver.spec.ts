...
describe("resolveShellForCommand", () => {
  it("runs the command through an interactive login shell and execs it on POSIX", () => {
    ...
    expect(shell).toEqual({
      file: "/bin/zsh",
      args: ["-l", "-i", "-c", "exec claude --session-id abc --model opus"],
    });
  });

  it("falls back to /bin/bash on linux when SHELL is unset", () => {
    const shell = resolveShellForCommand({ platform: "linux", env: {}, command: "codex --model gpt" });
    expect(shell).toEqual({ file: "/bin/bash", args: ["-l", "-i", "-c", "exec codex --model gpt"] });
  });
